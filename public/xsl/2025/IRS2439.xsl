    ...
    <div class="styBB" style="width:187mm;height:30.5mm;">
 	...
 	<xsl:choose>
      ...
      <div class="styIRS2439rightBox" style="width:75mm;height:30.5mm;text-align:center;">
        <span style="font-size:9pt;height:8mm;font-weight:bold;">Notice to Shareholder of Undistributed Long-Term Capital
         Gains</span>
        <br/>
        For calendar year 20
        <span class="styFixedUnderline" style="float:none;text-align:left;width:3mm">
			<xsl:choose>
				<xsl:when test="$Form2439Data/TaxPeriodBeginDt">
					<!-- DO NOTHING, BYPASS -->
				</xsl:when>
				<xsl:when test="$RtnHdrData/TaxPeriodBeginDt">
					<!-- DO NOTHING, BYPASS -->
				</xsl:when>
				<xsl:otherwise>
        ...
        <span style="height:3mm;">beginning 
			<span style="width:18mm; border-style: solid; border-top-width: 0px; border-bottom-width: 1px; 
             border-left-width: 0px; border-right-width: 0px;">
				<xsl:choose>
					<xsl:when test="$Form2439Data/TaxPeriodBeginDt">
        ...
        <span style="padding-top:0.25mm;height:3mm;">ending 
			<span style="width:27mm; border-style: solid; border-top-width: 0px; border-bottom-width: 1px; 
             border-left-width: 0px; border-right-width: 0px;">
				<xsl:choose>
					<xsl:when test="$Form2439Data/TaxPeriodEndDt">
						<xsl:call-template name="PopulateMonthDayYear">
						<xsl:with-param name="TargetNode" select="$Form2439Data/TaxPeriodEndDt"/>
						</xsl:call-template>
					</xsl:when>
					<xsl:when test="$RtnHdrData/TaxPeriodEndDt">
						<xsl:call-template name="PopulateMonthDayYear">
