...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
			...
			<body class="styBodyClass">
				<form id="Form5471ScheduleQ">
					...
					<div class="styStdDivLS" style="border-bottom:1px solid black;">
						<div class="styNameBox" style="width:150mm;height:9mm;">
						...
						<div class="styEINBox" style="width:30mm;height:9mm;padding-left:1mm;font-weight:normal;word-wrap:break-all">
					...
					<div class="styTableContainerLandscapeNBB" id="Page1Table" style="display:flex">
							...
							<thead class="styTableThead">
								<tr style="border-top:1px solid black;">
									...
									<th class="styTableCellHeader" scope="col" style="width:29mm;font-weight:normal;border-right-width:0px;">
										<strong>(vii)</strong><br /> Other Expenses <br /> (attach statement)
									</th>
								</tr>
							</thead>
							<tbody>
								...
								<tr>
									<td class="styTableCellTextInherit" style="border-bottom-width:0px;">
										...
										Other Foreign Personal Holding Company Income (Total) (attach statement -- see instructions)
										<span class="sty5471SQDotLn">.</span>
									</td>
									<xsl:call-template name="TotCellsSchQ">
					...
					<div class="styStdDivLS pageEnd" style="border-top:2px solid black;font-weight:bold;">
						...
						<span style="float:right;">Schedule Q (Form 5471) (Rev. 12-2024)</span>
					</div>
					<!-- Page 2 -->
					<div class="styStdDivLS">
						Schedule Q (Form 5471) (Rev. 12-2024)
					...
					<div class="styTableContainerLandscapeNBB" id="Page3Table" style="display:flex">
						<xsl:call-template name="SetInitialState"/>
						<table class="styTable" style="border-collapse:collapse;font-size:7pt;">
							<thead class="styTableThead">
								<tr style="border-top:1px solid black;">
						...
						<table class="styTable" style="border-collapse:collapse;font-size:7pt;">
							...
							<tbody>
								...
								<tr>
									...
									<xsl:call-template name="TotCellsSchQp2">
										<xsl:with-param name="TargetNode" select="$FormData/TotalTestedIncomeGrp"/>
										<xsl:with-param name="L3" select="true()"/>
									</xsl:call-template>
								</tr>
								<xsl:call-template name="UnitLineSchQp2">
									<xsl:with-param name="TargetNode" select="$FormData/TestedIncomeGrp"/>
					...
					<xsl:if test="$sep1f">
						<br/>
						<br/>
						<span class="styRepeatingDataTitle">Line 1f - Other Foreign Personal Holding Company Income (attach statement -- see instructions)</span>
						<xsl:call-template name="UnitLineSep">
							<xsl:with-param name="TargetNode" select="$FormData/OtherSubpartFIncomeGrp"/>
							<xsl:with-param name="Line">1f</xsl:with-param>
						</xsl:call-template>
	...
	<xsl:template name="UnitLineSchQp2">
		...
		<xsl:param name="L3" select="false()"/>
		...
		<xsl:for-each select="$TargetNode">
			<tr>
				...
				<td class="styTableCellAmtInherit" style="">
					<xsl:if test="$L3"><xsl:attribute name="style">background-color:lightgrey;</xsl:attribute></xsl:if>
					<xsl:if test="$L4"><xsl:attribute name="style">background-color:lightgrey;</xsl:attribute></xsl:if>
					<xsl:call-template name="PopulateAmount">
						<xsl:with-param name="TargetNode" select="LossAllocationAmt"/>
						<xsl:with-param name="MaxSize" select="17"/>
	...
	<xsl:if test="$Sep or (count($TargetNode) &lt; 1)">
		<tr>
			...
			<td class="styTableCellAmtInherit" style=""><xsl:if test="$L3"><xsl:attribute name="style">background-color:lightgrey;</xsl:attribute></xsl:if>
			<xsl:if test="$L4"><xsl:attribute name="style">background-color:lightgrey;</xsl:attribute></xsl:if>&nbsp;</td>
			<td class="styTableCellAmtInherit" style="border-right-width:0px;">&nbsp;</td>
		</tr>
	</xsl:if>
	<xsl:if test="$Sep or (count($TargetNode) &lt; 2)">
