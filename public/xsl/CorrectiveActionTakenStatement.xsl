				...
				<tbody>
					<xsl:for-each select="$TargetNode">
						<tr>
							...
							<td class="styDepTblCell" style="text-align:center;">
								<input type="checkbox" class="styCkbox" style="height:12px;width:12px;margin:0px;">
									<xsl:call-template name="PopulateCheckbox">
										<xsl:with-param name="TargetNode" select="Section4942h2ElectionInd"/>
										<xsl:with-param name="BackupName">CATSSection4942h2ElectionInd<xsl:value-of select="position()"/></xsl:with-param>
									</xsl:call-template>
									<xsl:attribute name="alt">Section 4942 h 2 Election Indicator <xsl:value-of select="position()"/></xsl:attribute>
								</input>
							</td>
						</tr>
					</xsl:for-each>
